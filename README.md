...
│   │   ├── verdict_graph.py         # LangGraph 멀티스테이지 오케스트레이션 (4종 분석 모드 + 검증·보정)
...
│       ├── rag/                     # 83개 (retriever 15 + reranker 14 + scorer 17 + verdict_graph 26 + verdict_store 11)
...
```bash
# 백엔드 전체 테스트 (288개 passed, 15개 skipped)
cd backend
...
LLM 판정 결과에 대한 **검증·일관성 검사·보정**을 LangGraph 그래프 워크플로우로 관리하며, 4종의 분석 모드를 지원합니다. 각 모드는 공통 기반(score → validate → correct 루프)을 공유하되, 검증 통과 후 모드별 전용 노드로 분기합니다.
...
- [x] LangGraph 멀티스테이지 오케스트레이션 (4종 분석 모드 + 역할별 프롬프트 분리 + 3중 검증·보정, `--no-graph`로 비활성화 가능)
- [x] 테스트 288개 passed, 15개 skipped
