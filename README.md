...
| 패키지 관리 | uv + pyproject.toml (백엔드 — Rust 기반 초고속 Python 패키지 매니저), npm (프론트엔드 — Node.js 표준 패키지 매니저) |
...
│   │   ├── verdict_graph.py         # LangGraph 멀티스테이지 오케스트레이션 (5종 분석 모드 + 검증·보정)
│   │   ├── verdict_store.py         # 판정 결과 JSONL 영속 저장/조회
│   │   ├── poll_store.py            # 여론조사 저장소 추상화 (Strategy + Registry)
│   │   ├── jsonl_poll_store.py      # JSONL 기반 여론조사 저장소 (로컬, 기본값)
│   │   └── gsheets_poll_store.py    # Google Sheets 기반 여론조사 저장소 (읽기 전용)
│   │
...
│       ├── ingestion/               # 148개 (scraper 40 + tagger 28 + chunker 37 + embedder 16 + pipeline 27)
│       ├── rag/                     # 132개 (retriever 15 + reranker 14 + scorer 33 + verdict_graph 29 + verdict_store 11 + poll_store 30)
...
# 후보 간 비교 분석 (--candidates 필수)
PYTHONPATH=. python -m rag.pipeline --district pyeongtaek_b --analysis comparison --candidates "김용남,유의동"

# 여론조사 분석 (Google Sheets 또는 JSONL 데이터 기반)
PYTHONPATH=. python -m rag.pipeline --district pyeongtaek_b --analysis opinion_polls
...
```bash
# 백엔드 전체 테스트 (344개 passed, 15개 skipped)
cd backend
...
여론조사 데이터는 두 가지 소스를 지원합니다:
- **JSONL (기본)**: 관리자 페이지(또는 API)를 통해 `data/polls.jsonl`에 저장
- **Google Sheets (읽기 전용)**: `polls_meta` + `polls_candidates` 시트를 `survey_id` FK로 JOIN하여 실시간 로드. `config.yaml`의 `polls.type: google_sheets`로 전환
...
### LangGraph 멀티스테이지 오케스트레이션

LLM 판정 결과에 대한 **검증·일관성 검사·보정**을 LangGraph 그래프 워크플로우로 관리하며, 5종의 분석 모드를 지원합니다. 각 모드는 공통 기반(score → validate → correct 루프)을 공유하되, 검증 통과 후 모드별 전용 노드로 분기합니다. `opinion_polls` 모드는 여론조사 전용 독립 경로를 사용합니다.

**5종 분석 모드 + 역할별 프롬프트**:

| 모드 | 프롬프트 | 그래프 플로우 | 출력 |
|------|---------|-------------|------|
| `verdict` (기본) | `VERDICT_PROMPT` | score → validate → END | 판세 판정 (우세/균형/열세, 승률) |
| `diagnosis` | `VERDICT_PROMPT` → `DIAGNOSIS_PROMPT` | score → validate → diagnose → END | 판정 + 후보별 문제점·원인 진단 |
| `strategy` | `VERDICT_PROMPT` → `DIAGNOSIS_PROMPT` → `STRATEGY_PROMPT` | score → validate → diagnose → strategize → END | 판정 + 진단 + 대응방안·전략 |
| `comparison` | `VERDICT_PROMPT` → `COMPARISON_PROMPT` | score → validate → compare → END | 판정 + 지정 후보 간 다차원 비교 |
| `opinion_polls` | `OPINION_POLLS_PROMPT` | analyze_polls → END | 여론조사 추세·방법론 분석 + 후보별 지지율 추이 |

모든 모드에서 공통 검증 루프가 동작합니다:

```
score 노드 → validate 노드 → pass → [모드별 분기]
                             → fail → correct 노드 → score 노드 (재시도, 최대 2회)
                             → max_retry → END (현재 판정 유지)
```
...
| 검증 | 내용 | 감지 예시 |
|------|------|----------|
| 근거 일치 | 근거 청크 0건인데 "우세" 판정, 미등록 후보 판정 | 조국 관련 청크 없는데 "우세" |
| 일관성 | 직전 판정 대비 승률 30%p 초과 급변 감지 | 45% → 80% (변동 35%) |
| 확률 범위 | 0~1 범위 초과, 합계 ≠ 1.0 | 합계 1.5 |

**노드별 상태 로깅**:
- `[score 노드]` — 진입(청크 수, retry 횟수), 완료(후보별 verdict·승률)
- `[validate 노드]` — 진입(직전 판정 유무), 검증별 통과/실패, 완료(오류 합계)
- `[correct 노드]` — 진입(오류 상세), 재시도 카운트 증가
- `[route]` — 분기 결정 사유 (통과/실패/최대재시도)
- `[diagnose/strategize/compare 노드]` — 진입(대상 후보), 완료(결과 건수)

```bash
# LangGraph 활성화 (기본값, verdict 모드)
PYTHONPATH=. python -m rag.pipeline --district pyeongtaek_b
...
# 후보 간 비교 모드 (--candidates 필수)
PYTHONPATH=. python -m rag.pipeline --district pyeongtaek_b --analysis comparison --candidates "김용남,유의동"

# 여론조사 분석 모드 (Google Sheets 또는 JSONL 데이터 기반)
PYTHONPATH=. python -m rag.pipeline --district pyeongtaek_b --analysis opinion_polls

# LangGraph 비활성화 (기존 단발 호출)
PYTHONPATH=. python -m rag.pipeline --district pyeongtaek_b --no-graph
```
...
- [x] LangGraph 멀티스테이지 오케스트레이션 (5종 분석 모드 + 역할별 프롬프트 분리 + 3중 검증·보정, `--no-graph`로 비활성화 가능)
- [x] Google Sheets 여론조사 연동 (PollStore Strategy+Registry 패턴, survey_id FK JOIN, 조사 방법론별 분석)
- [x] opinion_polls 분석 모드 (OPINION_POLLS_PROMPT — 추세·방법론·신뢰도 분석, analyze_polls → END 독립 경로)
- [x] 테스트 344개 passed, 15개 skipped
